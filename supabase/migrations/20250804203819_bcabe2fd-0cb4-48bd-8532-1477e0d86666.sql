...
CREATE INDEX idx_downloads_user_ip_date ON public.downloads(user_ip, DATE(downloaded_at));
...
BEGIN
  ...
  unique_downloads AS (
    SELECT DISTINCT ON (user_ip, software_name, DATE(downloaded_at))
      software_name,
      downloaded_at,
      user_ip
    ...
    ORDER BY user_ip, software_name, DATE(downloaded_at), downloaded_at
  )
  SELECT 
